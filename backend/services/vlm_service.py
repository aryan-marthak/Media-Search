...
import torch
from transformers import AutoProcessor, Idefics3Processor, Idefics3ForConditionalGeneration, BitsAndBytesConfig
...
class VLMService:
    ...
    def _load_model(self):
        ...
        try:
            ...
            self.processor = Idefics3Processor.from_pretrained(
                model_name,
            ...
            print(">> SmolVLM2-2.2B loaded successfully (4-bit optimized)")
            print(">> Deep Search is now available!")
            print(">> Performance: ~5-6 seconds per image")

        except KeyboardInterrupt:
            print(">> VLM loading interrupted — Deep Search disabled")
            self.model = None
            self.processor = None
    ...
    def generate_caption(
        ...
        try:
            ...
            # tokenize=False ensures a plain string is returned, not token IDs
            ...
            inputs = self.processor(
                text=str(prompt_text),  # defensive cast — always a string
                images=[image],
                return_tensors="pt",
            )

            # Use the model's actual device — device_map=auto may spread layers
